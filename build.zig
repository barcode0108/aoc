...
fn installExe(b: *Build, path: []const u8) !void {
...
fn listSrc(path: []const u8, b: *Build) !std.ArrayList([]const u8) {
    const p = b.path(path).getPath3(b, null);
    const dir = try p.openDir("", .{ .iterate = true });
    var it = dir.iterate();
    ...
    while (try it.next()) |entry| {
        ...
        if (strStartWith(entry.name, "aoc"))
            ret.append(entry.name) catch @panic("OOM");
    }

    return ret;
}

fn strStartWith(str: []const u8, cmp: []const u8) bool {
    if (str.len < cmp.len)
        return false;

    return std.mem.eql(u8, str[0..cmp.len], cmp);
}
